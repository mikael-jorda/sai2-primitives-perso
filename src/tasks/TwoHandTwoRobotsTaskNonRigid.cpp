/*
 * TwoHandTwoRobotsTaskNonRigid.cpp
 *
 *      Author: Mikael Jorda
 */

#include "TwoHandTwoRobotsTaskNonRigid.h"
...
TwoHandTwoRobotsTaskNonRigid::TwoHandTwoRobotsTaskNonRigid(Sai2Model::Sai2Model* robot_arm_1,
	...
	TwoHandTwoRobotsTaskNonRigid(robot_arm_1, robot_arm_2, link_name_1, link_name_2, 
		control_frame_1.translation(), control_frame_2.translation(),
		control_frame_1.linear(), control_frame_2.linear(), loop_time) {}
		            ...
		            const double loop_time)
{
	...
	if(dof_1 < 6 || dof_2 < 6)
	{
		throw std::invalid_argument("robots must have at least 6 DoF each for a TwoHandTwoRobotsTaskNonRigid\n");
	}
...
void TwoHandTwoRobotsTaskNonRigid::updateTaskModel(const Eigen::MatrixXd N_prec_1, const Eigen::MatrixXd N_prec_2)
{
	if(N_prec_1.rows() != N_prec_1.cols())
	{
		throw std::invalid_argument("N_prec_1 matrix not square in TwoHandTwoRobotsTaskNonRigid::updateTaskModel\n");
	}
	if(N_prec_1.rows() != _robot_arm_1->_dof)
	{
		throw std::invalid_argument("N_prec_1 matrix size not consistent with robot dof in TwoHandTwoRobotsTaskNonRigid::updateTaskModel\n");
	}
	if(N_prec_2.rows() != N_prec_2.cols())
	{
		throw std::invalid_argument("N_prec_2 matrix not square in TwoHandTwoRobotsTaskNonRigid::updateTaskModel\n");
	}
	if(N_prec_2.rows() != _robot_arm_2->_dof)
	{
		throw std::invalid_argument("N_prec_2 matrix size not consistent with robot dof in TwoHandTwoRobotsTaskNonRigid::updateTaskModel\n");
	}
...
void TwoHandTwoRobotsTaskNonRigid::computeTorques(Eigen::VectorXd& task_joint_torques_1, Eigen::VectorXd& task_joint_torques_2)
{
	...
	force_moment_contribution.tail(3) = 0 * moment_related_force;  // apply projection to the task force (linear only)

	// apply projection to inertnal task forces 
	internal_task_force.tail(5).setZero();

	// Eigen::VectorXd object_task_force = _Lambda_tot * position_orientation_contribution + force_moment_contribution - _object_gravity;
	Eigen::VectorXd object_task_force = _Lambda_tot * position_orientation_contribution - _object_gravity;
...
void TwoHandTwoRobotsTaskNonRigid::reInitializeTask()
{
	int dof_1 = _robot_arm_1->dof();
	int dof_2 = _robot_arm_2->dof();
...
void TwoHandTwoRobotsTaskNonRigid::setObjectMassPropertiesAndInitialInertialFrameLocation(double object_mass, Eigen::Affine3d T_world_com, Eigen::Matrix3d object_inertia)
{
...
void TwoHandTwoRobotsTaskNonRigid::setControlFrameLocationInitial(Eigen::Affine3d T_world_controlpoint)
{
...
void TwoHandTwoRobotsTaskNonRigid::setForceSensorFrames(const std::string link_name_1, const Eigen::Affine3d sensor_in_link_r1, 
									const std::string link_name_2, const Eigen::Affine3d sensor_in_link_r2)
{
	if(link_name_1 != _link_name_1  ||  link_name_2 != _link_name_2)
	{
		throw std::invalid_argument("The link to which is attached the sensor should be the same as the link to which is attached the control frame in TwoHandTwoRobotsTaskNonRigid::setForceSensorFrames\n");
	}
...
void TwoHandTwoRobotsTaskNonRigid::updateSensedForcesAndMoments(const Eigen::Vector3d sensed_force_sensor_frame_r1,
...
void TwoHandTwoRobotsTaskNonRigid::setForceAxis(const Eigen::Vector3d force_axis)
{
...
void TwoHandTwoRobotsTaskNonRigid::updateForceAxis(const Eigen::Vector3d force_axis)
{
	Eigen::Vector3d normalized_axis = force_axis.normalized();
...
void TwoHandTwoRobotsTaskNonRigid::setLinearMotionAxis(const Eigen::Vector3d linear_motion_axis)
{
...
void TwoHandTwoRobotsTaskNonRigid::updateLinearMotionAxis(const Eigen::Vector3d linear_motion_axis)
{
	Eigen::Vector3d normalized_axis = linear_motion_axis.normalized();
...
void TwoHandTwoRobotsTaskNonRigid::setMomentAxis(const Eigen::Vector3d moment_axis)
{
...
void TwoHandTwoRobotsTaskNonRigid::updateMomentAxis(const Eigen::Vector3d moment_axis)
{
	Eigen::Vector3d normalized_axis = moment_axis.normalized();
...
void TwoHandTwoRobotsTaskNonRigid::setAngularMotionAxis(const Eigen::Vector3d angular_motion_axis)
{
...
void TwoHandTwoRobotsTaskNonRigid::updateAngularMotionAxis(const Eigen::Vector3d angular_motion_axis)
{
	Eigen::Vector3d normalized_axis = angular_motion_axis.normalized();
...
void TwoHandTwoRobotsTaskNonRigid::setFullLinearMotionControl()
{
...
void TwoHandTwoRobotsTaskNonRigid::setFullForceControl()
{
...
void TwoHandTwoRobotsTaskNonRigid::setFullAngularMotionControl()
{
...
void TwoHandTwoRobotsTaskNonRigid::setFullMomentControl()
{
...
void TwoHandTwoRobotsTaskNonRigid::setClosedLoopForceControl()
{
	_closed_loop_force_control = true;
	_integrated_object_force_error.setZero();
}

void TwoHandTwoRobotsTaskNonRigid::setClosedLoopMomentControl()
{
	_closed_loop_moment_control = true;
	_integrated_object_moment_error.setZero();
}

void TwoHandTwoRobotsTaskNonRigid::setOpenLoopForceControl()
{
	_closed_loop_force_control = false;
}


void TwoHandTwoRobotsTaskNonRigid::setOpenLoopMomentControl()
{
	_closed_loop_moment_control = false;
}

void TwoHandTwoRobotsTaskNonRigid::resetIntegratorsLinear()
{
	_integrated_object_force_error.setZero();
	_integrated_object_position_error.setZero();
}

void TwoHandTwoRobotsTaskNonRigid::resetIntegratorsAngular()
{
	_integrated_object_moment_error.setZero();
	_integrated_object_orientation_error.setZero();
}

void TwoHandTwoRobotsTaskNonRigid::resetIntegrators()
{
	resetIntegratorsLinear();
	resetIntegratorsAngular();
}
