...
void PosOriTask::computeTorques(Eigen::VectorXd& task_joint_torques)
{
	...
	// update velocity and angular velocity for D term
	...
	// compute task force
	_task_force.head(3) = -_kp_pos*(_current_position - _desired_position) - _kv_pos*(_current_velocity - _desired_velocity) - _ki_pos*_integrated_position_error;
	_task_force.tail(3) = -_kp_ori*_orientation_error - _kv_ori*(_current_angular_velocity - _desired_angular_velocity) - _ki_ori*_integrated_orientation_error;
